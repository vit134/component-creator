...
const PROJECT_PATH = path.dirname(require.main.filename);
...
const getIndexJsConntent = async ({ dist, name, type, tmplType = 'clasic' }) => {
  const indexPath = `${PROJECT_PATH}/tmpl/${tmplType}/index_${type.toLowerCase()}_js.tmpl`;
...
const getPhaktJsConntent = async ({ name, tmplType = 'clasic' }) => {
  const phaktPath = `${PROJECT_PATH}/tmpl/${tmplType}/phakt_js.tmpl`;
  const componentName = utils.getComponentName(name)
...
const getSpecJsConntent = async ({ name, tmplType = 'clasic' }) => {
  const specPath = `${PROJECT_PATH}/tmpl/${tmplType}/spec_js.tmpl`;
  const componentName = utils.getComponentName(name)
